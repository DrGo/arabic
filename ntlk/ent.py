...
# /Users/drgo/local/code/ar/ntlk/samples/عن افكار لخ القايد.ar.md
with open('./samples/عن افكار لخ القايد.ar.md', 'r') as file:
    data = file.read()

ents = _extract_ner(text=data, model=model, tokenizer=tokenizer, start_token="▁")

for ent in ents:
    print("\t",ent["token"],"==>",ent["label"])
keys= ents[0].keys()
outf = open("output.csv", "w")
dict_writer = csv.DictWriter(outf, keys)
dict_writer.writeheader()
dict_writer.writerows(ents)
outf.close()
exit()
